...
const DemoUserDetails = () => {
  ...
  // Mark attendance - PUT request to update contact status
  const markAttendance = async (id) => {
    try {
      const url = `${API_BASE_URL}demoClass/demo-bookings/${id}/contact-status`;
      await axios.put(url, { is_contacted: true }, config);
      
      message.success("Attendance marked successfully!");
      
      // Refetch data to get updated is_contacted status
      await fetchDemousers(searchQuery);
    } catch (e) {
  ...
  const stats = useMemo(() => {
    const total = data.length || 0;
    const attended = data.filter(d => d.is_contacted === true).length || 0;
    const pending = total - attended;
  ...
  const exportToExcel = () => {
    const exportData = filteredData.map((item, index) => ({
      ...
      'Mobile': item.mobile_number || 'N/A',
      'Email': item.email || 'N/A',
      'Status': item.is_contacted ? 'Attended' : 'Not Attended'
    }));
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Button
          type={record.is_contacted ? "default" : "primary"}
          icon={record.is_contacted ? <CheckCircleOutlined /> : null}
          disabled={record.is_contacted}
          onClick={() => handleAttendance(record)}
          className={record.is_contacted ? "bg-green-50 border-green-300 text-green-600" : ""}
        >
          {record.is_contacted ? 'Attended' : 'Mark Attended'}
        </Button>
      ),
    },
  ];
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <Card className="shadow-md">
          <Table
            ...
            scroll={{ x: 1200 }}
            rowClassName={(record) => record.is_contacted ? 'bg-green-50' : ''}
          />
        </Card>
