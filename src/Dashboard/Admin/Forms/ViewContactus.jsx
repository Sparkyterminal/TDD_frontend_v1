...
import { Table, Card, Pagination, message, Input, Tag, Space, Statistic, Row, Col } from "antd";
import { SearchOutlined, PhoneOutlined, MailOutlined, UserOutlined, ClockCircleOutlined } from "@ant-design/icons";
...
const ViewContactus = () => {
  ...
  const todayContacts = data.filter(item => new Date(item.createdAt).setHours(0, 0, 0, 0) === today).length;
  ...
  const columns = [
    ...
    {
      title: <Space><PhoneOutlined /> Phone Number</Space>,
      dataIndex: "phone_number",
      key: "phone_number",
      render: (text) => <span style={{ fontFamily: 'monospace' }}>{text.toString()}</span>,
    },
    {
      ...
      render: (date) => {
        ...
        return (
          <div>
            ...
            <div style={{ fontSize: '12px', color: '#8c8c8c' }}>{d.toLocaleTimeString()}</div>
          </div>
        );
      },
  ...
  return (
    <div style={{ 
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', fontFamily: 'glancyr, sans-serif' }}>
        <div style={{ 
          borderRadius: '16px',
          padding: '32px',
          ...
          <p style={{ color: '#8c8c8c', marginBottom: '32px', fontSize: '16px', fontFamily: 'glancyr, sans-serif' }}>
          ...
          <Row gutter={24} style={{ marginBottom: 32 }}>
            <Col xs={24} sm={12} md={8}>
              ...
              >
                <Statistic 
                  title={<span style={{ color: 'rgba(255,255,255,0.9)' }}>Total Contacts</span>}
                  value={data.length}
            ...
            <Col xs={24} sm={12} md={8}>
              ...
              >
                <Statistic 
                  title={<span style={{ color: 'rgba(255,255,255,0.9)' }}>Today's Contacts</span>}
                  value={todayContacts}
                  valueStyle={{ color: '#fff', fontWeight: 700 }}
            ...
            <Col xs={24} sm={12} md={8}>
              ...
              >
                <Statistic 
                  title={<span style={{ color: 'rgba(255,255,255,0.9)' }}>Filtered Results</span>}
