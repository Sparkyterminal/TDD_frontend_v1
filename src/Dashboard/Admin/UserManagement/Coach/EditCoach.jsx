...
import { Form, Input, Button, Upload, message, Card, Divider, Spin } from "antd";
...
const EditCoach = () => {
  ...
  // Fetch coach details and prefill form
  useEffect(() => {
    const fetchCoachDetails = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${API_BASE_URL}user/${id}`, config);
        ...
        form.setFieldsValue({
          ...
          email_id: coachData.email_data.temp_email_id,
          role: "COACH",
        });

        // Handle existing image - following EditWorkshop pattern
        const firstMedia = Array.isArray(coachData.media) && coachData.media.length > 0 ? coachData.media[0] : null;
        const mediaThumb = firstMedia?.image_url?.thumbnail?.low_res || firstMedia?.image_url?.thumbnail?.high_res;
        const mediaId = firstMedia?.id || firstMedia?._id || coachData.image || null;
        
        if (mediaThumb || mediaId) {
          const imageUrl = mediaThumb 
            ? (mediaThumb.startsWith("http") ? mediaThumb : `${API_BASE_URL}${mediaThumb}`)
        ...
        messageApi.error("Failed to fetch coach details. Please try again.");
  ...
  const onFinish = async (values) => {
    ...
    try {
      const payload = {
        ...
        role: "COACH",
        media: [imageId || existingImageId], // Use new image ID or existing one
      };

      await axios.put(
        `${API_BASE_URL}user/edit/${id}`,
        payload,
        config
      );
      ...
      message.error("Failed to update instructor. Please try again.");
        ...
        <Form form={form} layout="vertical" onFinish={onFinish} className="w-full">
          <Card style={{ background: "#fff", border: "none", marginBottom: 24 }}>
            ...
            <Form.Item
              ...
              required
              validateStatus={!imageId && !existingImageId && fileList.length === 0 ? "error" : ""}
              help={!imageId && !existingImageId && fileList.length === 0 ? "Please upload an image" : ""}
              className="mb-0"
            >
              <Upload
                beforeUpload={() => false}
