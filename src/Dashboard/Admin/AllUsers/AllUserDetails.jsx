...
const AllUserDetails = () => {
  ...
  const getEndDate = (booking) => {
    ...
    // If end_date is not present, calculate based on renewal_date (preferred) or start_date
    const baseDate = booking.renewal_date || booking.start_date;
    if (baseDate && booking.billing_interval) {
      return calculateEndDate(baseDate, booking.billing_interval).toLocaleDateString("en-GB");
    }
    return null;
  };
  ...
  const handleBillingIntervalChange = (interval) => {
    const startDate = manualForm.getFieldValue('renewal_date');
    if (startDate) {
  ...
  const handleManualRenewalSubmit = async (values) => {
    try {
      ...
      const payload = {
        ...
        renewal_date: values.renewal_date.toISOString(),
  ...
  const handleReactivate = async (bookingId) => {
    try {
      setLoading(true);
      await axios.patch(
        `${API_BASE_URL}membership-plan/booking/${bookingId}/reactivate`,
        {},
        config
      );
    ...
    try {
      ...
      const bookingsData = filteredBookings.map((booking, index) => {
        ...
        return {
          ...
          startDate: booking.start_date ? new Date(booking.start_date).toLocaleDateString("en-GB") : null,
          renewalDate: booking.renewal_date ? new Date(booking.renewal_date).toLocaleDateString("en-GB") : null,
          endDate: endDate,
    ...
    try {
      ...
      const exportData = allData.map((booking, index) => {
        ...
        return {
          ...
          "Start Date": booking.start_date ? new Date(booking.start_date).toLocaleDateString("en-GB") : "N/A",
          "Renewal Date": booking.renewal_date ? new Date(booking.renewal_date).toLocaleDateString("en-GB") : "N/A",
  ...
  const handleEditUserSubmit = async (values) => {
    try {
      ...
      if (!editBookingData || !editBookingData.user._id || !editBookingData._id) {
        message.error("Invalid booking data");
        setEditUserLoading(false);
        return;
      }

      // Prepare payload to update user and booking in required structure
      const nameString = (values.name || '').trim();
      const [firstName, ...restNameParts] = nameString.split(' ');
