...
//     // Pass userId as a query parameter or state
...
//         userId: booking.userInfo?._id || booking.user, // Store userId
//       }));
...
const AllUserDetails = () => {
  ...
  const getEndDate = (booking) => {
    // Check if end_date exists in the booking data
    if (booking.end_date) {
      return new Date(booking.end_date).toLocaleDateString("en-GB");
    }

    // Calculate end date from start_date and billing_interval
    ...
    try {
      ...
      const bookingsData = res.data.data.bookings.map((booking, index) => {
        ...
        return {
          ...
          rawBooking: booking, // Store raw booking data for export
        };
      });
  ...
  const columns = [
    ...
    },
    {
      ...
      fixed: "right",
      render: (_, record) => {
        // If no end date available, show disabled button
        if (!record.endDate) {
          return (
        ...
        // For non-completed payments, always enable renewal
        ...
        // For completed payments, check if near end date or exceeded
        const renewalEnabled = isRenewalEnabled(record.endDate);
        const [day, month, year] = record.endDate.split('/');
        const endDate = new Date(year, month - 1, day);
        const currentDate = new Date();
