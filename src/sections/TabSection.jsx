...
//           alt={item.name}
//           className="w-full h-64 object-cover" // enlarged image height
...
//           >
//             <div>
//               <strong className="text-gray-800">Price:</strong> ₹{item.prices?.monthly ?? "-"} / month
//             </div>
...
//                         <li key={sch._id}>
//                           {sch.day} ({formatTimeToIST12Hour(sch.start_time)} - {formatTimeToIST12Hour(sch.end_time)})
//                         </li>
//                       ))}
//                     </ul>
//                   </div>
...
//       setLoading(true);
//       try {
//         const res = await axios.get(`${API_BASE_URL}membership-plan`, { params: { page: 1, limit: 100 } });
//         const data = res.data || {};
...
//   const danceClasses = items.filter(i => i.dance_type?.category === "dance classes" && i.is_active);
//   const groupClasses = items.filter(i => i.dance_type?.category === "group classes" && i.is_active);
//   const kidsJunior = items.filter(i => i.plan_for === "KID" && i.kids_category === "JUNIOR" && i.is_active);
//   const kidsAdvanced = items.filter(i => i.plan_for === "KID" && i.kids_category === "ADVANCED" && i.is_active);
...
//   return (
//     <section className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 ${fontClass}`}>
...
//             activeTab === "dance_classes" ? "bg-gray-900 text-white" : "bg-gray-200 text-gray-800"
//           }`}
//         >
//           Dance Classes
//         </button>
...
//           className={`px-4 py-2 rounded-full ${
//             activeTab === "group_classes" ? "bg-gray-900 text-white" : "bg-gray-200 text-gray-800"
//           }`}
//         >
//           Group Classes
//         </button>
...
//           className={`px-4 py-2 rounded-full ${
//             activeTab === "kids" ? "bg-gray-900 text-white" : "bg-gray-200 text-gray-800"
//           }`}
//         >
//           Kids
//         </button>
...
//             >
//               {loading ? <p className="text-center">Loading...</p> : renderGrid(danceClasses)}
//             </motion.div>
//           )}
...
//               transition={fadeSlideVariants.transition}
//             >
//               {loading ? <p className="text-center">Loading...</p> : renderGrid(groupClasses)}
//             </motion.div>
//           )}
...
//                     kidsTab === "JUNIOR" ? "bg-gray-900 text-white" : "bg-gray-200"
//                   }`}
//                 >
//                   Junior
//                 </button>
...
//                   className={`px-3 py-2 rounded-full ${
//                     kidsTab === "ADVANCED" ? "bg-gray-900 text-white" : "bg-gray-200"
//                   }`}
//                 >
//                   Advanced
//                 </button>
...
//               {loading ? <p className="text-center">Loading...</p> : kidsTab === "JUNIOR" ? renderGrid(kidsJunior) : renderGrid(kidsAdvanced)}
//             </motion.div>
//           )}
//         </AnimatePresence>
//       </div>
...
const staticGroupClasses = [
  {
    name: "Aerobics",
    image: {
      image_url: { thumbnail: { low_res: "assets/dance/aerobic.jpg" } },
    },
  },
  {
    name: "HIIT",
    image: {
      image_url: { thumbnail: { low_res: "assets/dance/hiit.jpg" } },
    },
  },
  {
    name: "Pilates",
    image: { image_url: { thumbnail: { low_res: "assets/dance/pilates.jpg" } } },
  },
  {
    name: "yoga",
    image: {
      image_url: { thumbnail: { low_res: "assets/dance/yoga.jpg" } },
    },
  },
  {
    name: "Zumba",
    image: {
      image_url: { thumbnail: { low_res: "assets/dance/zumba.jpg" } },
    },
  },
  {
    name: "Dance Fitness",
    image: { image_url: { thumbnail: { low_res: "assets/dance/dance-workshop.jpg" } } },
  },
];
...
const TabSection = () => {
    ...
    const fetch = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          // Extract first group classes ID from API data
  ...
  // Static cards for group classes with name, image, and ages text + button below
const renderStaticGroupCards = () => (
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {staticGroupClasses.map(({ name, image }, idx) => (
          <div
            key={idx} // no _id as static cards don't have ID
            className="bg-white rounded-xl shadow-md p-4 flex flex-col h-full"
          >
            <div className="w-full overflow-hidden rounded-md mb-4">
              <img
                src={image.image_url.thumbnail.low_res}
                alt={name}
